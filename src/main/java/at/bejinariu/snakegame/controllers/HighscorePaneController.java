...
public class HighscorePaneController implements Initializable {
    ...
    @FXML
    private TableColumn<Highscore, String> colBorders;
    @FXML
    private TableColumn<Highscore, String> colSpeed;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        colBorders.setCellValueFactory(
                cell -> new SimpleStringProperty(cell.getValue().bordersEnabledProperty().get() ? "Yes" : "No"));
        colScore.setCellValueFactory(
                cell -> new SimpleStringProperty(cell.getValue().playerScoreProperty().getValue() + pointsExtension));
        colPlayer.setCellValueFactory(
                cell -> cell.getValue().playerNameProperty());
        colSpeed.setCellValueFactory(
                cell -> new SimpleStringProperty(cell.getValue().playerSpeedProperty().get().toString()));
    ...
    public void readEntries() {
        ...
        try {
            listScores.addAll(
                    Files.lines(new File("src/main/resources/data/scores.csv").toPath())
